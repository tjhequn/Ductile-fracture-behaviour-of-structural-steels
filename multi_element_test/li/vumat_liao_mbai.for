...
c
      real*4 coordmp(nblock,*), charlength(nblock), props(nprops),
...
C without considering the effect of triaxility
C         phi = 1.0d0
C         dphideta = 0.0d0
...
C without considering the effect of lode angle
        gamma = psi_const*(1.0d0/cos(pi*theta/6.0d0)-1.0d0)
        ...
        psi = ctheta_s+(ctheta_ax-ctheta_s)*(gamma-gamma**(m+1.0d0)/(m+1.0d0))
        psi_norm = ctheta_s+m*(ctheta_t-ctheta_s)/(m+1.0d0)
        psi = psi/psi_norm

        dgammadtheata = psi_const*tan(pi*theta/6.0d0)/cos(pi*theta/6.0d0)*pi/6.0d0
        dpsidtheta = (ctheta_ax-ctheta_s)*(1-gamma**m)*dgammadtheata/psi_norm

C C without considering the effect of lode angle
C         psi = 1.0d0
C         dpsidtheta = 0.0d0
...
C----------------------------------------------------------------------
      subroutine fractureStrian(fracturePara,eta,theta,fstrain)
        ...
        include 'vaba_param_dp.inc'
        real*4 :: fracturePara(6), D1,D2,D3,D4,D5,D6
...
C----------------------------------------------------------------------
        ...
        fstrain = (D1*exp(-D2*eta)-D3*exp(-D4*eta))*theta**2.0d0+
     1             D3*exp(-D4*eta)
